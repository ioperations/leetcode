...
class UndergroundSystem {
   public:
    ...
    void CheckIn(int id, string station_name, int t) {
        ...
        Node tmp;
        tmp.m_station = std::move(station_name);
        tmp.m_time = t;
        m_wait[id] = tmp;
    }
...
class Solution2 {
   public:
    ...
    unordered_map<string, pair<long long, int>> m_station_times;
...
TEST(DesignUndergroundSystem, t1) {
    /*
Input
...
TEST(DesignUndergroundSystem, t2) {
    auto* rgs = new Solution2();
...
TEST(DesignUndergroundSystem, t3) {
    auto* rgs = new Solution2();
