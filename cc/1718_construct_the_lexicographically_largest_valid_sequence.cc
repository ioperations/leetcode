...
class Solution {
   public:
    vector<int> constructDistancedSequence(int n) {
        ...
        for (int i = n; i > 1; i--) {
            valueSet.push_back(i);
        }

        Gen(results, valueSet, exists, false);
        return results;
    }
    ...
    bool Gen(vector<int>& results, const vector<int>& valueSet,
        ...
        if (fillsize == resultSize) {
            return true;
        }

        if (results[index] != 0) {
            return Gen(results, valueSet, exists, numberOneUsed, index + 1,
                       fillsize);
        }
        ...
        for (size_t i = 0; i < valueSet.size(); i++) {
            ...
            const auto value = valueSet[i];
            if (index + value >= resultSize || results[index + value] != 0) {
                continue;
            }
            ...
            if (Gen(results, valueSet, exists, numberOneUsed, index + 1,
                    fillsize + 2)) {
                return true;
            }
        ...
        results[index] = 1;
        if (Gen(results, valueSet, exists, true, index + 1, fillsize + 1)) {
            return true;
        }
        results[index] = 0;
        return false;
...
TEST(t0, t1) {
    int n = 3;
    std::vector<int> output = {3, 1, 2, 3, 2};
...
TEST(t0, t2) {
    int n = 5;
    std::vector<int> output = {5, 3, 1, 4, 3, 5, 2, 4, 2};
    // Explanation: [2,3,2,1,3] is also a valid sequence, but [3,1,2,3,2] is the
    // lexicographically largest valid sequence.
...
TEST(t0, t3) {
    int n = 6;
    std::vector<int> output = {6, 4, 2, 5, 2, 4, 6, 3, 5, 1, 3};
