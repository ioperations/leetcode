...
// number i has the label labels[i]). The edges array is given on the form
// edges[i] = [ai, bi], which means there is an edge between nodes ai and bi in
// the tree. Return an array of size n where ans[i] is the number of nodes in
// the subtree of the ith node which have the same label as node i. A subtree of
// a tree T is the tree consisting of a node in T and all of its descendant
// nodes.
...
mod tests {
    ...
    fn case1_test() {
        ...
        // label 'a' as well, thus the answer is 2. Notice that any node is part
        // of its sub-tree. Node 1 has a label 'b'. The sub-tree of node
        // 1 contains nodes 1,4 and 5, as nodes 4 and 5 have different labels
    ...
    fn case2_test() {
        ...
        // thus the answer is 2. The sub-tree of node 0 contains nodes
        // 0, 1, 2 and 3, all with label 'b', thus the answer is 4.
        let ret = Solution::count_sub_trees(n, &edges, labels);
        assert_eq!(ret, output);
    }
