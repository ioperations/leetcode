...
// multiple times) with the following restrictions: After you sell your stock,
// you cannot buy stock on the next day (i.e., cooldown one day). Note: You may
// not engage in multiple transactions simultaneously (i.e., you must sell the
        ...
        if day == prices.len() {
            ...
            *profit
            ...
            let rez = match invested {
                Invested::Yes => Self::dp(prices, day + 1, invested, memo).max(
                    prices[day]
                        + Self::dp(prices, day + 1, Invested::Cooldown, memo),
                ),
                Invested::No => Self::dp(prices, day + 1, invested, memo).max(
                    -prices[day]
                        + Self::dp(prices, day + 1, Invested::Yes, memo),
                ),
                Invested::Cooldown => {
                    Self::dp(prices, day + 1, Invested::No, memo)
                }
            };
