...
// of length m. Return an array answer of length m where answer[i] is the
// maximum size of a subsequence that you can take from nums such that the sum
// of its elements is less than or equal to queries[i]. A subsequence is an
...
mod tests {
    ...
    fn case1_test() {
        ...
        // It can be proven that 2 is the maximum size of such a subsequence, so
        // answer[0] = 2. The subsequence [4,5,1] has a sum less than or
        // equal to 10. It can be proven that 3 is the maximum size of
        // such a subsequence, so answer[1] = 3. The subsequence
        // [4,5,2,1] has a sum less than or equal to 21.
    ...
    fn case2_test() {
        ...
        let queries = [1];
        let output = [0];
        // The empty subsequence is the only subsequence that has a sum less
